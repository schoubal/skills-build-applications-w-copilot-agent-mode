...
function Users() {
  ...
  const fetchData = () => {
    ...
    console.log('[Users] fetching endpoint:', url);
    setIsLoading(true);
    ...
    <div className="card shadow-sm mb-4">
      <div className="card-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
        <div>
          ...
          <p className="mb-0 text-muted">User records from the Django REST API.</p>
        </div>
      ...
      <div className="card-body">
        <form className="row g-2 align-items-center mb-3" onSubmit={(e) => e.preventDefault()}>
          <div className="col-md-8">
            ...
            <input
              ...
              placeholder="Filter users by text..."
            />
          </div>
        ...
        {isLoading ? (
          ...
          <div className="alert alert-warning">No users match the current filter.</div>
        ) : (
          filteredData.map((item, index) => (
            <div key={index} className="card mb-3">
              <div className="card-header d-flex justify-content-between align-items-center">
                <span className="fw-semibold">User {index + 1}</span>
                <button
                  type="button"
